...
public class CarsSheet {
	...
	private final String CITY_COLUMN = "Prowincja";
	...
	private final String MILEAGE_COLUMN = "Przebieg [km]";
	private final String PRICE_COLUMN = "Cena";
	private final String PRICECUR_COLUMN = "Waluta";

	public void export(WritableWorkbook workbook, List<CarItem> cars) throws CrawlerExportException {
		WritableSheet sheet = workbook.createSheet("Oferty", 1);
		createHeader(sheet);
		try {
			fillWithData(cars, sheet, sheet.getRows());
	...
	private void fillWithData(List<CarItem> cars, WritableSheet sheet, int startRow) throws RowsExceededException, CrawlerExportException {
		try {
			...
			for (CarItem car : cars) {
				sheet.setRowView(i, 1000);
					sheet.addCell(new Label(1, i, car.getDealer().getProvince(), arial10format));
					sheet.addCell(new Label(2, i, car.getDealer().getName(), arial10format));
					...
					sheet.addCell(new Label(6, i, car.getPrzebieg(), arial10format));
