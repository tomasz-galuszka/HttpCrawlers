...
public class Connector {
	...
	private String userAgent = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:15.0) Gecko/20120427 Firefox/17.0a1";
	public static String baseUrl = "http://www.autowereld.nl";
	private String baseQuery = "/bedrijf/provincie-%s/p%d/";
	private Config config;

	public Document getAllDealersDocument(String province, int page) throws ConnectionException, ConfigException {
		Document document = null;
		try {
			String requestAddress = baseUrl + String.format(baseQuery, province.toLowerCase(), page);
			...
			throw new ConnectionException("Blad podczas pobierania strony www.\nUruchom program w konsoli i sprawdź logi.", e);
	...
	private InputStream connect(String www) throws IOException, ConnectionException, ConfigException {
		...
		try {
			...
			throw new ConnectionException("Niepoprawny adres url, sprawdz czy adres: " + www + " istnieje.\n Uruchom program w konsoli i sprawdź logi.", e);
		}
		return connection.getInputStream();
	}

	private Config getConfig() throws ConnectionException, ConfigException {
		if (config == null) {
			config = new Config();
		}
		return config;
