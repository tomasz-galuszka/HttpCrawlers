...
import com.crawler.autovit.CrawlerException;
import com.crawler.autovit.data.beans.ProductItem;

public class XlsExporter {

	private String outputXls = "rumunia_autovit.xls";
	private final String ROOT_DIR = "baza";
	private final String LP_COLUMN_NAME = "Lp.";
	private final String PROVINCE_COLUMN_NAME = "Hrabstwo";
	...
	private final String CURRENCY_COLUMN_NAME = "Waluta";
	...
	public void createXls(boolean fileExist) throws CrawlerException {
		try {
			...
			throw new CrawlerException("Nie można utworzyć wynikowego pliku xls", e);
	...
	public void appendToFile(XlsPackage xlsPackage) throws CrawlerException {
		File destFile = getDestFilePath();
		...
		try {
			...
			throw new CrawlerException("Nie mozna odczytac wynikowego pliku xls", e);
		} catch (WriteException e) {
			throw new CrawlerException("Nie mozna zapisac do wynikowego pliku xls", e);
		} catch (IOException e) {
			throw new CrawlerException("Problem ze znalezieniem pliku xls", e);
		}

	}

	private void createHeader(WritableSheet sheet) throws CrawlerException {
		...
		try {
			...
			throw new CrawlerException("Blad podczas tworzenia naglowka tabeli w pliku xls", e1);
		}
		try {
			...
			sheet.addCell(new Label(8, 1, CURRENCY_COLUMN_NAME, arial14format));
		} catch (RowsExceededException e) {
			throw new CrawlerException("Blad podczas tworzenia nagloka tabeli w pliku xls, przekroczony limit wierszy", e);
		} catch (WriteException e) {
			throw new CrawlerException("Blad podczas tworzenia naglowka tabeli w pliku xls, problem z zapisem", e);
		}
	}
	...
	private void fillWithData(XlsPackage xlsPackage, WritableSheet sheet, int startRow) throws WriteException, RowsExceededException {
		...
		for (ProductItem p : products) {
			...
			sheet.addCell(new Label(7, i, String.valueOf(p.getPrice()), arial10format));
			sheet.addCell(new Label(8, i, p.getPriceCurrency().trim(), arial10format));
			i++;
		}
	...
	private File getCopyDestFilePath() {
		...
		String tmp_file = outputXls.replace(".xls", "_tmp.xls");
		File destFile = new File(rootDir.getAbsolutePath() + File.separator + tmp_file);
		return destFile;
	}
	...
	public String getLastExportedDealerName() throws CrawlerException {
	...
	public String getLastExportedDealerProvince() throws CrawlerException {
		if (!isFilePresent(getDestFilePath())) {
			return null;
		}
		try {
	...
	public void removeTrailingRows(String lastDealerName) throws CrawlerException {
		...
		try {
			...
			throw new CrawlerException("Usuwanie nieukonczonego dealera z listy. Nie mozna odczytac wynikowego pliku xls", e);
		} catch (WriteException e) {
			throw new CrawlerException("Usuwanie nieukonczonego dealera z listy. Nie mozna zapisac do wynikowego pliku xls", e);
		} catch (IOException e) {
			throw new CrawlerException("Usuwanie nieukonczonego dealera z listy. Nie mozna znalezc pliku wynikowego pliku xls", e);
		}
	}
